      ...
      <Section className="pt-0">
        <Container>
          ...
          <motion.article
            ...
            className="group grid overflow-hidden rounded-3xl border border-border-base bg-surface lg:grid-cols-2"
          >
            <div className={cn('relative flex h-64 items-center justify-center bg-gradient-to-br lg:h-auto', featured.gradient)}>
              <div className="absolute inset-0 bg-dots opacity-40" />
              <span className="rounded-full bg-background/70 px-4 py-1.5 text-sm font-semibold text-foreground backdrop-blur">
                Featured
              </span>
            </div>
            <div className="flex flex-col justify-center gap-4 p-8 sm:p-10">
              <div className="flex items-center gap-3 text-sm text-muted">
                <span className="font-semibold text-ember-500">{featured.category}</span>
                <span>•</span>
                <span>{formatDate(featured.date)}</span>
                <span className="flex items-center gap-1"><FiClock className="h-3.5 w-3.5" /> {featured.readingTime} min</span>
              </div>
              <h2 className="font-display text-2xl font-bold text-foreground sm:text-3xl">{featured.title}</h2>
              <p className="text-muted">{featured.excerpt}</p>
              <button className="inline-flex w-fit items-center gap-1.5 font-semibold text-ember-500 hover:gap-2.5">
          ...
          <div className="mt-12 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-2 rounded-full border border-border-strong bg-surface px-4 py-2 lg:w-72">
          ...
          >
            {filtered.map((post) => (
              <motion.article
                key={post.slug}
                variants={fadeInUp}
                className="group flex flex-col overflow-hidden rounded-3xl border border-border-base bg-surface transition-all duration-300 hover:-translate-y-1 hover:border-ember-400/40"
              >
                <div className={cn('relative h-40 bg-gradient-to-br', post.gradient)}>
                  <div className="absolute inset-0 bg-dots opacity-40" />
                  <span className="absolute left-4 top-4 rounded-full bg-background/70 px-3 py-1 text-xs font-semibold text-foreground backdrop-blur">
                    {post.category}
                  </span>
                </div>
